...
contract('ShreyToken', function(accounts) {
    var tokenInstance;
    ...
    it('initializes the contract with the correct values', function() {
        return ShreyToken.deployed().then(function(instance) {
            ...
            assert.equal(name, 'SHrey Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'SHREY', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard) {
            assert.equal(standard, 'SHrey Token v1.0', 'has the correct standard');
        });
    });
    ...
    it('transfers token ownership', function() {
      return ShreyToken.deployed().then(function(instance) {
        tokenInstance = instance;
        // Ensure failure by transferring more than the total supply
        return tokenInstance.totalSupply();
      }).then(function(supply) {
        return tokenInstance.transfer(accounts[1], supply.toNumber() + 1, { from: accounts[0] });
